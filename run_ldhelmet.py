	...
	for bpen in [5, 100]:
		...
		for ix, chr_list in enumerate(chrs):
			...
			for chr in chr_list:
				...
				haplo = '/mnt/gluster/home/sonal.singhal1/%s/analysis/LDhelmet/%s_haplotypes.fasta' % (species, chr)
				out = '/mnt/gluster/home/sonal.singhal1/%s/analysis/LDhelmet/%s_recombination_bpen%s' % (species, chr, bpen)
				prior = rhos[chr]
				max_lk = prior * 2

				if chr == 'chrZ':
					pade = '/mnt/gluster/home/sonal.singhal1/%s/analysis/LDhelmet/%s_chrZ.pade' % (species, species)
                                        lk = '/mnt/gluster/home/sonal.singhal1/%s/analysis/LDhelmet/%s_chrZ.gen' % (species, species)
				else:
					pade = '/mnt/gluster/home/sonal.singhal1/%s/analysis/LDhelmet/%s_auto.pade' % (species, species)
                			lk = '/mnt/gluster/home/sonal.singhal1/%s/analysis/LDhelmet/%s_auto.gen' % (species, species)
			...
			print "echo \'sh %s\' | qsub -l h_vmem=15g -cwd -V -j y -N \'%s_%s_%s\'" % (outsh, species, bpen, ix)
