...
for species in ['ZF']:
	...
	for ix, chr_list in enumerate(chrs):
		...
		o = open(outsh, 'w')
		for chr in chr_list:
			aa = '/mnt/gluster/home/sonal.singhal1/%s/ancestral_allele/with_fam/ancestral_allele.%s.ldhelmet.txt' % (species, chr)
			haplo = '/mnt/gluster/home/sonal.singhal1/%s/phasing/PIR_approach/with_fam/%s_haplotypes.fasta' % (species, chr)
			out = '/mnt/gluster/home/sonal.singhal1/%s/analysis/LDhelmet/with_fam/maps/%s_recombination_bpen%s' % (species, chr, bpen)
			prior = rhos[chr]
			max_lk = prior * 2	
