...
out = '/mnt/gluster/home/sonal.singhal1/%s/analysis/mut_skew/summary_mutskew_rho.csv' % sp
	...
	for l in f:
		...
		if af < 1 and af > 0:
			if rho_bin not in skew:
				skew[rho_bin] = {}
				for t in types:
					skew[rho_bin][t] = {'rho': 0, 'num': 0, 'af': 0}
			skew[rho_bin][type]['rho'] += rho
			skew[rho_bin][type]['num'] += 1
			skew[rho_bin][type]['af'] += af
	f.close()
...
for rho_bin in skew:
	for type in skew[rho_bin]:
		if skew[rho_bin][type]['num'] > 0:
			avg_af = skew[rho_bin][type]['af'] / float( skew[rho_bin][type]['num'] )
			avg_rho = skew[rho_bin][type]['rho'] / float( skew[rho_bin][type]['num'] )
			o.write('%s,%s,%s,%s,%s,%s\n' % (rho_bin, rho_vals[rho_bin], type, avg_af, avg_rho, skew[rho_bin][type]['num'] ))
o.close()
