...
def get_divergence(start, end, hap1, nind1, hap2, nind2, keep_sites):
	...
	for i in range(nind1):
		...
		for site in sites:
			if site in hap1:
				...
				# append the reference if not variable
				hap += keep_sites[site]
		haps1.append(hap)

	haps2 = []
        for i in range(nind2):
                hap = ''
                for site in sites:
                        if site in hap2:
                                hap += hap2[site][i]
	...
	for i in range(nind1):
		for j in range(nind2):
			...
			diff = [x == y for (x, y) in zip(haps1[i], haps2[j])].count(False)
			if  len(keep_sites) > 0:
				pi_sum += (1/float(nind1)) * (1/float(nind2)) * (diff/float(len(keep_sites)))
	return pi_sum
...
def get_divergence_within(start, end, hap, nind, keep_sites):
        ...
        for i in range(nind):
                for j in range(i):
                        # does this handle missing data correctly? no.
                        diff = [x == y for (x, y) in zip(haps[i], haps[j])].count(False)
                        if  len(keep_sites) > 0:
                                pi_sum += (1/float(nind)) * (1/float(nind)) * (diff/float(len(keep_sites)))
	pi_sum = pi_sum * 2
...
def main():
	...
	out_file = '/mnt/gluster/home/sonal.singhal1/ZF/analysis/pop_gen/%s.%s_%s.divergence.csv' % (chr, sp1, sp2)
