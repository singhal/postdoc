...
hotspot_files = glob.glob('/mnt/gluster/home/sonal.singhal1/simulations/hotspot_simulations/sim_10_20_40_60_80_100/hotspots/*')

tack_on = 'bpen'
outfile = '%s%s.hotspot_detection.csv' % (dir, tack_on)
out = open(outfile, 'w')
out.write('real_rho,theta,real_length,bpen,actual_rate_diff,estimated_rate_diff,false_pos\n')

outfile1 = '%s%s.hotspot_power.csv' % (dir, tack_on)
out1 = open(outfile1, 'w')
out1.write('bpen,theta,rho,cutoff,power\n')

abs_cut_off = 5
cut_off = {10: 24, 20: 24, 40: 24, 60: 24, 80: 24, 100: 24} 
# cut_off = {2: 6, 3: 6, 5: 6, 10: 6, 50: 6, 100: 6}
# cut_off = {5: 100, 10: 100, 20: 100, 40: 100, 60: 100, 80: 100, 100: 100}
...
for hotspot_file in hotspot_files:
	...
	maps = glob.glob('%smaps/*%s_*hotspots*' % (dir, type))
	...
	for map in maps:
		...
		if bpen not in found_hotspots[rho][theta]:
			found_hotspots[rho][theta][bpen] = []
		...
		for spot in hotspots:
			num_false = 0
			# false positive
			poss_hotspots = d[d.diff_rate > abs_cut_off]
			...
			for start, end in hotspots[spot]:
				...
				if match.diff_rate >= abs_cut_off:
					found_hotspots[rho][theta][bpen].append(spot)
...
for rho in found_hotspots:
	for theta in found_hotspots[rho]:
		for bpen in found_hotspots[rho][theta]:
			for cut in cut_off:
				count = found_hotspots[rho][theta][bpen].count(cut)
