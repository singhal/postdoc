...
def get_hs_cs(d):
	...
	for chr, hot_mid, cold_mid in zip(d.chr, d.hot_mid, d.cold_mid):
...
def get_lineage_mutations(seq, counts):
	...
	for ix, (anc, zf, ltf, dbf) in enumerate(izip(seq['anc'], seq['ZF'], seq['LTF'], seq['DBF'])):
		tuple1anc = tuple2anc = tuple1der = tuple2der = 'NN'
			...
			tuple1der = seq['DBF'][ix - 1] + dbf
                if (ix + 1) < len(seq['anc']):
                        tuple2anc = anc + seq['anc'][ix + 1]
			tuple2der = dbf + seq['DBF'][ix + 1]
...
def main():
	...
	out = '/mnt/gluster/home/sonal.singhal1/ZF/analysis/bgc_hotspots/DBF.hotspots_noCpG.csv'
	o = open(out, 'w')
	o.write('chr,midpoint,spot_type,bin,ancAT,ancGC,AT_GC,GC_AT\n')
	spots = '/mnt/gluster/home/sonal.singhal1/ZF/analysis/bgc_hotspots/matched_hotspots.01GC.001CpG.DBF.csv'
